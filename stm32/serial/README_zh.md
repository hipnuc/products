...
3. 设备需启用支持的输出：二进制 HI91/HI81/HI83，或 NMEA 的 GGA/RMC。
   例程最多每 200 ms 显示一次新样本，每两秒报告接收情况、无数据或恢复接收。
...
`hipnuc_board_send_command("LOG HI91 ONTIME 0.01")` 通过 PA2 发送 ASCII 命令；
例程本身不调用，只有需要发送命令时才接 PA2。

[hipnuc_board.c](USER/hipnuc_board.c) 包含串口初始化和接收实现，占用 USART2
及其中断、作为控制台的 USART1/PA9、1 ms SysTick，以及 `HIPNUC_BOARD_USE_DMA`
为 1 时的 DMA1 通道 6；`hipnuc_board_init()` 还会设置 NVIC 优先级分组 2。
不要在其他位置重复配置这些资源。
...
检测到覆盖后会增加溢出计数，并丢弃缓冲中的字节和未完成帧。中断被阻塞期间，
