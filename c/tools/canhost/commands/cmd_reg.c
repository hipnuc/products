...
static int parse_args(int argc, char **argv, reg_args_t *out)
{
    ...
    out->target_count = config_get_target_nodes(out->target_nodes, CONFIG_MAX_NODES);
    ...
    if (args.do_read) {
        for (int i = 0; i < args.target_count; ++i) {
            ...
            if (got < 0) {
                ...
                printf("addr=0x%04X val=%u (0x%08X) status=%u\n", args.addr,
                       (unsigned)res.value, (unsigned)res.value, (unsigned)res.status);
            }
        }
