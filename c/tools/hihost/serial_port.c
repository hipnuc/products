...
const int serial_port_baud_rates[] = {
    115200, 921600, 9600, 460800, 230400, 57600, 38400, 19200, 4800, 0
};
...
int serial_port_open(const char *portname)
{
    ...
    int n = snprintf(full_name, sizeof(full_name), "%s%s",
                     strncmp(portname, "/dev/", 5) == 0 ? "" : "/dev/", portname);
    if (n < 0 || (size_t)n >= sizeof(full_name)) {
        fprintf(stderr, "Error: port name too long\n");
        return -1;
    }
...
int serial_port_write(int fd, const void *buffer, int size)
{
    ...
    int bytes_written = (int)write(fd, buffer, (size_t)size);
    if (bytes_written < 0)
    {
        return -1;
    }
...
int serial_port_read_timeout(int fd, void *buffer, int size, int timeout_ms)
{
    ...
    // Wait for data or timeout
        ...
        int bytes_read = (int)read(fd, buf + total_read, (size_t)(size - total_read));
        if (bytes_read < 0) 
        {
            perror("read");
            return -1;
 ...
 */
static int serial_send_then_recv(int fd, const uint8_t *tx_data, int tx_len,
...
int serial_send_then_recv_str(int fd, const char *send_str, const char *expected, char *recv_buf, size_t recv_buf_size, int timeout_ms)
{
    ...
    int send_len = (int)strlen(send_str);
        ...
        while ((ent = readdir(dir)) != NULL && count < MAX_PORTS) {
            ...
            if (is_physical_serial_port(ent->d_name)) {
                ...
                if (stat(path, &st) == 0 && S_ISCHR(st.st_mode)) {
                    ...
                    int n = snprintf((*ports)[count].name, MAX_PORT_NAME, "%s", path);
                    if (n > 0 && n < MAX_PORT_NAME) {
                        count++;
                    }
                }
            }
